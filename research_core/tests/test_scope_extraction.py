...
pytestmark = pytest.mark.skipif(
    not os.environ.get("OPENAI_API_KEY"), reason="Regime 2 is opt-in; needs OPENAI_API_KEY"
)
...
def test_complex_scope_stable_fields():
    sp = parse_scope(
        {
            "project_description": "Adding a coating booth and storing 60 gallons of a flammable solvent at a "
            "Los Angeles County manufacturing facility."
        },
        "eval-complex",
    )
