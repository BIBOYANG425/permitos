...
def test_verdict_parity(golden):
    ...
    assert canonical(_as_dicts(out["verification_verdicts"])) == canonical(
        golden["verification_verdicts"]
    )
...
def test_determinations_parity(golden):
    ...
    plan = plan_research(golden["scope_pack"], [])
    result = finalize_run(
        golden["run_id"], golden["scope_pack"], plan, golden["fixture_evidence"], [], []
    )
