...
def _result(model, recall, grounding, accuracy, total_cost, cpd_p50):
    return {
        ...
        "scorecard": {
            "evaluators_primary": {
                "expected_program_recall": recall,
                "grounding_faithfulness": grounding,
            },
...
def test_recommend_excludes_models_below_floor():
    results = [
        _result("cheapo", 1.0, 0.5, 0.40, 0.02, 0.0003),  # grounding below floor
        _result("gpt-5.2", 1.0, 1.0, 0.62, 0.15, 0.002),
    ]
...
def test_render_report_md_includes_models_and_recommendation():
    from research_aiq.optimize import render_report_md

    results = [
        _result("gpt-5.2", 1.0, 1.0, 0.62, 0.15, 0.002),
        _result("gpt-5.5", 1.0, 1.0, 0.70, 0.55, 0.007),
    ]
    md = render_report_md(results, "gpt-5.2")
    assert "gpt-5.2" in md and "gpt-5.5" in md
    assert "Cost-optimal" in md and "gpt-5.2" in md.split("Cost-optimal")[1]
